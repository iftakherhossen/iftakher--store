  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `
      <div class="single-product mx-2 my-3">
        ...
        <p><i class="fas fa-star"></i> ${rating.rate} &nbsp; <i class="fas fa-shopping-bag"></i></i> ${rating.count}</p>
        <h3>Price: $ ${product.price}</h3>
        <div class="d-flex justify-content-evenly mt-4 mb-1">
          <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn mt-auto">Add to cart</button>
          <button id="details-btn" class="btn mt-auto">Details</button>
        </div>
      </div>
    `;
...
//     <option value="electronics">${productCategory}</option>
//     <option value="jewelery">${productCategory}</option>
//     <option value="men's clothing">${productCategory}</option>
//     <option value="women's clothing">${productCategory}</option>
//   `; 
// }
...
const searchFilter = category => {
  ...
  document.getElementById('category').addEventListener('change', function () {
    let productWrapper = document.getElementById('all-products')
    productWrapper.textContent = '';

    if (this.value) {
      products = allProductsData.filter(c => c.category == this.value);
      console.log(products);
